 ...
 * Customer workspace: portal (17009) — business customer self-service.
 */

export type WorkspaceKey = 'admin' | 'crm' | 'billing' | 'desk' | 'portal' | 'services';

export const WORKSPACE_URLS = {
  admin:   process.env.NEXT_PUBLIC_ADMIN_URL   ?? 'http://localhost:17005',
  crm:     process.env.NEXT_PUBLIC_CRM_URL     ?? 'http://localhost:17006',
  billing: process.env.NEXT_PUBLIC_BILLING_URL ?? 'http://localhost:17007',
  desk:    process.env.NEXT_PUBLIC_DESK_URL    ?? 'http://localhost:17008',
  portal:   process.env.NEXT_PUBLIC_PORTAL_URL   ?? 'http://localhost:17009',
